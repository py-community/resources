...
// initialize the map
  var map = L.map('map', {zoomControl: false}).setView([10, 20],2);
...
if($("#pyladies_heat_map").length){

d3.csv("https://raw.githubusercontent.com/py-community/pyladies/master/docs/data/pyladies.csv").then(function(data) {
...
var n=echarts.init(document.getElementById("pyladies_heat_map"),a);n.setOption({title:{text:"PyLadies Global Community",subtext:"Data from Meetup.com", tooltip:{trigger:"item",formatter:function(a){var b=(a.value+"").split(".");return b=b[0].replace(/(\d{1,3})(?=(?:\d{3})+(?!\d))/g,"$1,"),a.seriesName+"<br/>"+a.name+" : "+b}},toolbox:{show:!0,orient:"vertical",x:"right",y:"center",feature:{mark:{show:!0},dataView:{show:!0,title:"Text View",lang:["Text View","Close","Refresh"],readOnly:!1},restore:{show:!1,title:"Restore"},saveAsImage:{show:!0,title:"Save Image"}}},dataRange:{show:!0,min:0,max:30,color:["#ff94e7","#FFFFFF"]},series:[{name:"PyLadies Groups ",type:"map",mapType:"world",roam:!0,scaleLimit:{min:1.28},mapLocation:{y:50},itemStyle:{emphasis:{label:{show:!0}}}, data:JSON.parse(dat2)
}]})
}); // d3.csv call
} //end if pyladies_heat_map


if($("#pyladies_country_chart").length){
...
var c1=echarts.init(document.getElementById("pyladies_country_chart"),a);c1.setOption({title:{text:"",subtext:""},color:["red"],tooltip:{trigger:"axis"},legend:{x:220,y:40,data:["Number of Chapters"]},toolbox:{show:!0,feature:{magicType:{show:!0,title:{line:"Line",bar:"Bar"},type:["line","bar"]},restore:{show:!0,title:"Restore"},saveAsImage:{show:!0,title:"Save Image"}}},calculable:!0,xAxis:[{type:"category",axisLabel:{
...
}  //if pyladies_country_chart



if($("#pyladies_top_us_dest").length){
d3.csv("https://raw.githubusercontent.com/py-community/pyladies/master/docs/data/pyladies.csv").then(function(data) {
   var cleandata = data.map(function(d) {
    var cleanD = {};
    d3.keys(d).forEach(function(k) {
...
uscity = [];
usmembers = [];
   for(let i = 0, l = us_sort_members.length; i < l; i++) {
	uscity.push(us_sort_members[i].key);
     usmembers.push(us_sort_members[i].value.members);
}

var fus1=echarts.init(document.getElementById("pyladies_top_us_dest"),a);fus1.setOption({title:{text:"US/Canada",subtext:"All Cities"},color:["#ba1f1f"],tooltip:{trigger:"axis"},calculable:!0,xAxis:[{type:"value"}],yAxis:[{type:"category",boundaryGap:!0, axisLabel:{ interval:0 }, data:uscity.reverse()  }],grid: { left: 100 },
series:[{name:"Number of PyLadies",type:"bar",smooth:!1, barWidth:7,itemStyle:{}, data:usmembers.reverse()   }]})


eucity = [];
eumembers = [];
   for(let i = 0, l = eu_sort_members.length; i < l; i++) {
	eucity.push(eu_sort_members[i].key);
     eumembers.push(eu_sort_members[i].value.members);
}

var feu=echarts.init(document.getElementById("pyladies_top_eu_dest"),a);feu.setOption({title:{text:"Europe",subtext:"All Cities"},color:["#ba1f1f"],tooltip:{trigger:"axis"},calculable:!0,xAxis:[{type:"value"}],yAxis:[{type:"category",boundaryGap:!0, axisLabel:{ interval:0 }, data:eucity.reverse()  }],grid: { left: 100 },
series:[{name:"Number of PyLadies",type:"bar",smooth:!1, barWidth:12,itemStyle:{}, data:eumembers.reverse()   }]})
	
	
latamcity = [];
latammembers = [];
   for(let i = 0, l = latam_sort_members.length; i < l; i++) {
	latamcity.push(latam_sort_members[i].key);
     latammembers.push(latam_sort_members[i].value.members);
}

var flat=echarts.init(document.getElementById("pyladies_top_latam_dest"),a);flat.setOption({title:{text:"Latin America",subtext:"All Cities"},color:["#ba1f1f"],tooltip:{trigger:"axis"},calculable:!0,xAxis:[{type:"value"}],yAxis:[{type:"category",boundaryGap:!0, axisLabel:{ interval:0 }, data:latamcity.reverse()  }],grid: { left: 100 },
series:[{name:"Number of PyLadies",type:"bar",smooth:!1, barWidth:16,itemStyle:{}, data:latammembers.reverse()   }]})
...
asiacity = [];
asiamembers = [];
   for(let i = 0, l = asia_sort_members.length; i < l; i++) {
	asiacity.push(asia_sort_members[i].key);
     asiamembers.push(asia_sort_members[i].value.members);
}

var fasia=echarts.init(document.getElementById("pyladies_top_asia_dest"),a);fasia.setOption({title:{text:"Asia",subtext:"All Cities"},color:["#ba1f1f"],tooltip:{trigger:"axis"},calculable:!0,xAxis:[{type:"value"}],yAxis:[{type:"category",boundaryGap:!0, axisLabel:{ interval:0 }, data:asiacity.reverse()  }],grid: { left: 100 },
series:[{name:"Number of PyLadies",type:"bar",smooth:!1, barWidth:18,itemStyle:{}, data:asiamembers.reverse()   }]})

//console.log(af_sort_members)

auscity = [];
ausmembers = [];
   for(let i = 0, l = au_sort_members.length; i < l; i++) {
	auscity.push(au_sort_members[i].key);
     ausmembers.push(au_sort_members[i].value.members);
}

var faus=echarts.init(document.getElementById("pyladies_top_aus_dest"),a);faus.setOption({title:{text:"Australia",subtext:"All Cities"},color:["#ba1f1f"],tooltip:{trigger:"axis"},calculable:!0,xAxis:[{type:"value"}],yAxis:[{type:"category",boundaryGap:!0, axisLabel:{ interval:0 }, data:auscity.reverse()  }],grid: { left: 100 },
series:[{name:"Number of PyLadies",type:"bar",smooth:!1, barWidth:35,itemStyle:{}, data:ausmembers.reverse()   }]})

afrcity = [];
afrmembers = [];
   for(let i = 0, l = af_sort_members.length; i < l; i++) {
	afrcity.push(af_sort_members[i].key);
     afrmembers.push(af_sort_members[i].value.members);
}

var fafr=echarts.init(document.getElementById("pyladies_top_afr_dest"),a);fafr.setOption({title:{text:"Africa",subtext:"All Cities"},color:["#ba1f1f"],tooltip:{trigger:"axis"},calculable:!0,xAxis:[{type:"value"}],yAxis:[{type:"category",boundaryGap:!0, axisLabel:{ interval:0 }, data:afrcity.reverse()  }],grid: { left: 100 },
series:[{name:"Number of PyLadies",type:"bar",smooth:!1, barWidth:35,itemStyle:{}, data:afrmembers.reverse()   }]})
...
d3.json("https://raw.githubusercontent.com/py-community/pyladies/master/docs/data/pyladies_summary.json").then(function(data) {
 regiondata = JSON.stringify(data[1]);
 chapters  = [data[1][0].us_can, data[1][0].eur, data[1][0].latinAm, data[1][0].aus, data[1][0].asia, data[1][0].afr];
 regions = ["US/Canada","Europe","Latin America","Australia","Asia","Africa"]
...
var h=echarts.init(document.getElementById("member_pie"),a);h.setOption({tooltip:{trigger:"item",formatter:"{a} <br/>{b} : {c} ({d}%)"},legend:{x:"center",y:"bottom",data: region },toolbox:{show:!0,feature:{magicType:{show:!0,type:["pie","funnel"]},restore:{show:!0,title:"Restore"},saveAsImage:{show:!0,title:"Save Image"}}},calculable:!0,series:[{name:"Number of PyLadies ",type:"pie",radius:[45,170],center:["50%",190],roseType:"radius",x:"50%",max:40,sort:"ascending",data: member_data }]})
}
...
d3.json("https://raw.githubusercontent.com/py-community/pyladies/master/docs/data/pyladies_cumulative.json").then(function(data) {
//console.log(data);
 dates = [];
 counts = []
...
var f2=echarts.init(document.getElementById("cumulative_chart"),a);f2.setOption({title:{text:"",subtext:""},color:["#ba1f1f"],tooltip:{trigger:"axis"},legend:{x:220,y:40,data:["Cumulative Count of Chapters"]},toolbox:{show:!0,feature:{magicType:{show:!0,title:{line:"Line",bar:"Bar"},type:["line","bar"]},restore:{show:!0,title:"Restore"},saveAsImage:{show:!0,title:"Save Image"}}},calculable:!0,xAxis:[{type:"category", axisLabel:{
...
var MM = ((date.getMonth() + 1) < 10 ? '0' : '') + (date.getMonth() + 1);
    ...
    }, boundaryGap:!1,data: dates}],yAxis:[{type:"value", splitArea:{show:!0,areaStyle:{color:["#ffffff","#eabbbb"]}}
